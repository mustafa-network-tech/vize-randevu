...
--
-- KURULUM SIRASI (bağımlılık sırasına göre):
--   1. set_updated_at()   — bağımsız yardımcı
--   2. profiles           — auth.users'a bağlı
--   3. is_admin()         — profiles'a bağlı  ← PROFILES'tan SONRA gelir
--   4. handle_new_user()  — profiles'a insert
--   5. auth trigger       — handle_new_user'a bağlı
--   6. profiles RLS       — is_admin()'e bağlı
--   7. visa_accounts      — profiles'a bağlı
--   8. bots               — visa_accounts'a bağlı
--   9. bot_logs           — bots'a bağlı
--  10. notifications      — profiles'a bağlı
--  11. appointments       — bots'a bağlı
--  12. index'ler
-- =============================================================================


-- ===========================================================================
-- 1. set_updated_at() — bağımsız
-- ===========================================================================
...
-- ===========================================================================
-- 2. profiles — auth.users referansı
-- ===========================================================================
...
-- ===========================================================================
-- 3. is_admin() — profiles TABLOSU oluştuktan SONRA
--    LANGUAGE plpgsql kullanılır: tablo referansı çalışma zamanında çözülür,
--    fonksiyon oluşturulurken "relation does not exist" hatası oluşmaz.
-- ===========================================================================
...
-- ===========================================================================
-- 4. handle_new_user() + auth trigger
--    Not: Supabase'de auth şemasına trigger eklemek için
--    Dashboard > SQL Editor çalışır, doğrudan psql gerektirebilir.
-- ===========================================================================
...
COMMENT ON FUNCTION public.handle_new_user() IS
  'auth.users INSERT sonrası otomatik profil oluşturur.';

-- Trigger: auth.users → profiles otomatik oluşturma
...
-- ===========================================================================
-- 5. profiles RLS — is_admin() hazır olduktan sonra tanımlanır
-- ===========================================================================
...
COMMENT ON COLUMN public.visa_accounts.encrypted_password IS 'Uygulama katmanında şifrelenmiş parola. Ham değer saklanmaz.';

ALTER TABLE public.visa_accounts ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "visa_accounts_select" ON public.visa_accounts;
DROP POLICY IF EXISTS "visa_accounts_insert" ON public.visa_accounts;
DROP POLICY IF EXISTS "visa_accounts_update" ON public.visa_accounts;
DROP POLICY IF EXISTS "visa_accounts_delete" ON public.visa_accounts;
...
CREATE TABLE IF NOT EXISTS public.bots (
  ...
  check_interval   integer     NOT NULL DEFAULT 60
                               CHECK (check_interval BETWEEN 10 AND 3600),
...
COMMENT ON COLUMN public.bots.check_interval IS 'Tarama aralığı, saniye (10–3600).';
COMMENT ON COLUMN public.bots.auto_book      IS 'Slot bulunduğunda otomatik rezervasyon.';
...
DROP POLICY IF EXISTS "bots_delete" ON public.bots;
...
COMMENT ON COLUMN public.bot_logs.id IS 'bigint IDENTITY — yüksek yazma hızı için.';

ALTER TABLE public.bot_logs ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "bot_logs_select" ON public.bot_logs;
DROP POLICY IF EXISTS "bot_logs_insert" ON public.bot_logs;
DROP POLICY IF EXISTS "bot_logs_delete" ON public.bot_logs;
...
COMMENT ON COLUMN public.notifications.type IS
  'slot_found | bot_error | captcha | proxy_changed | account_locked | appointment_booked';
